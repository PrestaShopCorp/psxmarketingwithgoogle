...
final class Upgrade200HookRegistrationTest extends TestCase
{
    public function testRegistersOnlyMissingRequiredHooksAndIsIdempotent(): void
    {
        $module = new UpgradeHookModuleDouble(['displayHeader']);

        self::assertTrue(\psxmgUpgrade200RegisterHooks($module, [
            'displayHeader',
            'moduleRoutes',
        ]));
    ...
    {
        $module = new UpgradeHookModuleDouble([], ['moduleRoutes']);

        self::assertFalse(\psxmgUpgrade200RegisterHooks($module, ['moduleRoutes']));
    }
}

final class UpgradeHookModuleDouble
{
    ...
    /**
     * @param string[] $activeHooks
     * @param string[] $failingHooks
     */
